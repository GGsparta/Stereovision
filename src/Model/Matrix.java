...
class Matrix {
    private HashMap<Pair<Integer,Integer>,MatrixPoint3D> map; // The map contains all the matrix's points, one for each (X,Y) pair
    ...
    void setPoint(int x, int y, double z) {
        var p = findPoint(x,y);
        if (p != null) p.add(0, 0, z - p.getZ());
    }
